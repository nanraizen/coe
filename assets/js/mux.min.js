...
$('.navbar-fixed').after('<div class="navbar-fixed-divider"></div>');
...
$(function () {
    ...
    var btnSlideContent = $('.btnSlideContent');
    var arrowLeft = $('.btnSlideArrow .prev');
    var arrowRight = $('.btnSlideArrow .next');
    var slideStep = 200;

    btnSlide.on('scroll', function () {
        var slideLeft = btnSlide.scrollLeft();
        var containerWidth = btnSlide.width();
        var contentWidth = btnSlideContent.width();
        var slideRight = slideLeft + containerWidth;

        if (slideRight >= contentWidth - 1) {
            arrowRight.parent().addClass('inactive');
        } else {
            arrowRight.parent().removeClass('inactive');
        }

        if (slideLeft <= 0) {
            arrowLeft.parent().removeClass('active');
        } else {
            arrowLeft.parent().addClass('active');
        }
    });

    arrowLeft.on('click', function () {
        btnSlide.animate({
            scrollLeft: '-=' + slideStep
        }, 300);
    });
